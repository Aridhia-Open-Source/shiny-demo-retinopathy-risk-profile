...
shinyServer(function(input, output, session) {
  ...
  output$patient_banner <- renderUI({
    #gender <- dataset$gender[dataset$retinal_id == input$select_id][1]
    gender <- dataset$gender[dataset$retinal_id == ids[values$i]][1]
    HTML(paste0('<div id="textbox"><p class="alignleft" style="font-size:20px"><b>',
              #dataset$name[dataset$retinal_id == input$select_id][1],
  ...
  filename_left <- reactive({paste0('images/', 
                                    #input$select_id,
                                     ids[values$i],
                                    '_left.jpeg')
  }) 
  
  filename_right <- reactive({paste0('images/',
                                      #input$select_id,
                                      ids[values$i],
                                      '_right.jpeg')
  })
  ...
  output$choose_symbol <- renderUI({
    #if (dataset$score[dataset$retinal_id == input$select_id][1] == 0){
    ...
    #} else if (dataset$score[dataset$retinal_id == input$select_id][1] == 1){
    ...
    #} else if (dataset$score[dataset$retinal_id == input$select_id][1] == 2){
    ...
    #} else if (dataset$score[dataset$retinal_id == input$select_id][1] == 3){
    ...
    #} else if (dataset$score[dataset$retinal_id == input$select_id][1] == 4){
  ...
  output$clin_history = renderDataTable({
    #dt <- dataset[dataset$retinal_id == input$select_id, c("event_dt", "location", "clinician", "event_type", "description", "event_result", "event_measure_id")]
    ...
    dt %>% filter(is.na(Date) == F) %>% arrange(desc(Date)) %>% mutate(Result = round(Result, 1), Code = round(Code, 1))
    
  }, options = list(#lengthMenu = c(5), 
                    pageLength = 5,
                    #paging = FALSE,
                    searching = FALSE,
                    bLengthChange = FALSE,
